...
-- New user account registration
	...
	DROP PROCEDURE IF EXISTS newUser;
		-- DELITING TEST INSERT
			DELETE FROM Customer WHERE email = 'randol@uark.edu';
	...
	-- Test
	CALL newUser('randol@uark.edu', 'Randol Watson', '212-903-8909', '10001','Park Row', 'New York', 'New York', 'LetsGetThoseBooks', @error, @accID);
	SELECT @error AS 'Error', @accID AS 'Account ID';
...
-- User login
	...
	-- Test
	CALL logIn('randol@uark.edu', 'LetsGetThoseBooks', @accID);
	SELECT @accID AS 'User Account';
	

-- User can update his address, password etc.
	
	UPDATE Customer 
		SET zipCode = 15004, street = 'South Street', city =  'Atlasburg', state = 'Pennsylvania ',
		    password = MD5('IJustGotThoseBooks'), phoneNo = '610-333-666'
		WHERE accID = 6;
...
-- Add/delete books from shopping cart
	...
	DELETE FROM cartContBooks
		WHERE ISBN = '978-0-596-15806-4' AND cartID = 1;
...
-- Turn shopping carts to orders
	...
	CREATE PROCEDURE cartToOrder(
		IN accID INTEGER UNSIGNED, 
		IN varCartID INTEGER UNSIGNED, 
		IN billAddr CHAR(40), 
		IN payMethod CHAR(12), 
		IN shipAddr CHAR(40), 
		OUT varOrdID INTEGER UNSIGNED)
	BEGIN
		DECLARE ordDate DATE;
		DECLARE totalPrice FLOAT;

		SELECT CONVERT(NOW(), DATE) INTO ordDate;

		INSERT INTO placedOrder(payMethod, billAddr, shipAddr, shipDate, orDate, status, accID)
		VALUES(payMethod, billAddr, shipAddr, NULL, ordDate, 'Pending to Pay', accID);

		SELECT LAST_INSERT_ID() INTO  varOrdID;

		INSERT INTO OrdContBook(ordID, quantity, ISBN)
			SELECT varOrdID, C.quantity, C.ISBN FROM cartContBooks C
				WHERE cartID = varCartID;

		SELECT SUM( O.quantity * B.price) 
			FROM ordContBook O, Book B
			WHERE O.ordID = varOrdID AND O.ISBN = B.ISBN INTO totalPrice;

		UPDATE placeOrder
			SET totalPrice = totalPrice
			WHERE ordID = varOrdID;

		DELETE FROM cartContBooks WHERE cartID =  varCartID;
		DELETE FROM ShoppingCartManage WHERE cartID = varCartID;
	END $$
...
-- Insert new books
	INSERT INTO Books (category, autName , year, ISBN, title, edit, publisher, stock, price)
		VALUES ( 'Science Fiction & Fantasy', 'George R.R Martin', 1996, '978-1-5247-9628-0',
			  'Fire and Blood: 300 Years Before A Game of Thrones’, 1, 'Bantam Spectra', 10, 15.98);
...
-- Obtained reward points
	UPDATE Customer
		SET rwdPoint = rwdPoint + (SELECT P.totalPrice * 0.1 
			FROM placeOrder P
			WHERE P.accID = 1 AND P.orderID = 2)
		WHERE accID = 1;
